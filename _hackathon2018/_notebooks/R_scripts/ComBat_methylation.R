...
modcombat = model.matrix(~as.factor(cat_gender)+as.numeric(cat_age)+as.numeric(cat_packs), data=pheno_data) # +as.numeric(cat_age)+as.numeric(cat_packs), cat_smoke_years
...
if(param){parstring='param'}else{parstring='nonparam'}
  ...
  if (nullmode==TRUE){
    if (splitByType==FALSE){
        corrected_batch_only_par = CComBat(full_matrix, batch = batch, mod = mod0, par.prior = param)
        ...
        corrected_batch_only_par = CComBat(full_matrix1, batch = batch1, mod = mod01, par.prior = param)
        write.table(corrected_batch_only_par, 
                    file=paste("/media/koekiemonster/DATA-FAST/genetic_expression/hackathon_2/Lung/_prepped/methylation_combat_0nans_primaryonly_",
                               parstring, "_target1.csv", sep=""),
                    row.names=TRUE)
        
        corrected_batch_only_par = CComBat(full_matrix2, batch = batch2, mod = mod02, par.prior = param)
        write.table(corrected_batch_only_par, 
                    file=paste("/media/koekiemonster/DATA-FAST/genetic_expression/hackathon_2/Lung/_prepped/methylation_combat_0nans_primaryonly_",
      ...
      if (splitByType==FALSE){
        corrected_all_parametric = CComBat(full_matrix, batch = batch, mod = modcombat, par.prior = param)
        write.table(corrected_all_parametric, 
                    file=paste("/media/koekiemonster/DATA-FAST/genetic_expression/hackathon_2/Lung/_prepped/methylation_combat_0nans_primaryonly_",
                               parstring, "_gender.csv", sep=""), 
                    row.names=TRUE)
      }else{
        corrected_all_parametric = CComBat(full_matrix1, batch = batch1, mod = modcombat1, par.prior = param)
        write.table(corrected_all_parametric, 
                    file=paste("/media/koekiemonster/DATA-FAST/genetic_expression/hackathon_2/Lung/_prepped/methylation_combat_0nans_primaryonly_",
                               parstring, "_gender_target1.csv", sep=""), 
                    row.names=TRUE)
        
        corrected_all_parametric = CComBat(full_matrix2, batch = batch2, mod = modcombat2, par.prior = param)
        write.table(corrected_all_parametric, 
                    file=paste("/media/koekiemonster/DATA-FAST/genetic_expression/hackathon_2/Lung/_prepped/methylation_combat_0nans_primaryonly_",
                               parstring, "_gender_target2.csv", sep=""),
                    row.names=TRUE)
      }
    }
