...
# [x] GSOM
# [] blob-drawer plus Picasso https://github.com/pachterlab/picasso/blob/main/Picasso.py 
...
class GSOM(BaseEstimator, TransformerMixin):
    
    def __init__(self, spred_factor, dimensions, distance='euclidean', 
                 initialize='random', learning_rate=0.3,
                 smooth_learning_factor=0.8, training_iterations=100, 
                 smooth_iterations=25, max_radius=6, FD=0.1, 
                 r=3.8, alpha=0.9, index_col=None, label_col=None,
    ...
    def winner_identification_and_neighbourhood_update(self, data_index, data, radius, learning_rate):
        out = sc.spatial.distance.cdist(self.node_list[:self.node_count], data[data_index, :].reshape(1, self.dimentions), self.distance)
    ...
    def fit(self, data):
        ...
        for i in tqdm(range(self.training_iterations)):
            radius_exp = self._get_neighbourhood_radius(self.training_iterations, i)
            if i != 0:
        ...
        for i in tqdm(range(self.smooth_iterations)):
            radius_exp = self._get_neighbourhood_radius(self.training_iterations, i)
            if i != 0:
                current_learning_rate = self._get_learning_rate(current_learning_rate)
        ...
        # Identify winners
        out = sc.spatial.distance.cdist(self.node_list[:self.node_count], 
                                            data, self.distance)

        self.train_assignments=out.argmin(axis=0)
        return self
    ...
    def predict(self, data):
        ...
        output_columns = [self.index_col]
        if self.label_col:
            weight_columns.remove(self.label_col)
            output_columns.append(self.label_col)

        weight_columns.remove(self.index_col)
        data_n = data[weight_columns].to_numpy()
        data_out = pd.DataFrame(data[output_columns])
        # Identify winners
        out = sc.spatial.distance.cdist(self.node_list[:self.node_count], 
                                            data_n, self.distance)
        data_out["output"] = out.argmin(axis=0)

        grp_output =data_out.groupby("output")
        dn = grp_output[self.index_col].apply(list).reset_index()
        dn = dn.set_index("output")
        if self.label_col:
            dn[self.label_col] = grp_output[self.label_col].apply(list)
        dn = dn.reset_index()
        dn["hit_count"] = dn[self.index_col].apply(lambda x: len(x))
